...
#         # 提取 graph-level labels
#         y = torch.stack([data.y for data in batch], dim=0)  # [batch_size, 9]
#
#         # 清空每个 data.y，避免 Batch 自动拼接
#         for data in batch:
#             data.y = None
#
#         # 构建 batch
...
#     )
#
#     # train_loader = DataLoader(Dataset(is_train=True), batch_size=bsz, shuffle=True, collate_fn=collate_fn)
#     # test_loader = DataLoader(Dataset(is_train=False), batch_size=bsz, shuffle=False, collate_fn=collate_fn)
#
...
#             loss = loss_fn(pred, y)
#             print("pred shape:", pred.shape)  # [batch_size, 9]
#             print("batch_data.y shape:", batch_data.y.shape)  # [batch_size, 9]
...
# def generate_params():
#     distributions = {
#         'STEPS': scipy.stats.randint(200, 500),
#         'LR': scipy.stats.loguniform(1e-5, 1e-3),
...
#     }
#     params = {key: val.rvs(1).item() for key, val in distributions.items()}
#     return params
#
#
