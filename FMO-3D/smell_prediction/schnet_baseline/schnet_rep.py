...
class SchNet_Classify(SchNet):
    def __init__(
        ...
        **kwargs
        ...
        super().__init__(n_atom_basis, n_interactions, radial_basis, cutoff_fn, n_filters, shared_interactions, activation, nuclear_embedding, electronic_embeddings)
        self.num_labels = num_labels
        # 定义 48 个二分类头，每个是 Dense(2, softmax)
        self.classifiers = [
            nn.Sequential(
                nn.Linear(n_atom_basis, 2, bias=False),
                nn.Softmax(dim=1),
            )
    ...
    def forward(self, inputs: Dict[str, torch.Tensor]):
        ...
        classify_result = torch.zeros(inputs["scalar_representation"].shape[0], self.num_labels, device=inputs["scalar_representation"].device)
        for i, classifier in enumerate(self.classifiers):
            classify_result[:, i] = classifier(inputs["scalar_representation"])[:, 1]
